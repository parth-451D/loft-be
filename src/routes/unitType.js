...
router.get("/unit-type/:id", async (req, res) => {
  try {
    const [rows, fields] = await pool.query(
      "SELECT * FROM unit_types where id = ?",
      [req.params.id]
    );
    if (rows.length > 0) {
      res
        .status(200)
        .json({ UnitTypeData: rows, message: "Unit Type fetch successfully" });
    } else {
      res
        .status(400)
        .json({ message: "Unit Type does not exist with this ID" });
    }
  } catch (err) {
    res.status(400).json({ message: err.message });
  }
...
// Get all flat
router.get("/unit-type", async (req, res) => {
  try {
    const [rows, fields] = await pool.query("SELECT * FROM unit_types");
    if (rows.length === 0) {
