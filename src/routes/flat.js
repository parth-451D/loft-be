    ...
    if (duplicate.length > 0) {
      ...
      try {
        ...
        const [rows, fields] = await pool.query(
          "INSERT INTO flats (floorId, unitNo, unitType, cleaningFees, description, images, price, bathrooms, beds, guests, deposit) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
          [
            req.body.floorId,
            req.body.unitNo,
            req.body.unitType,
...
router.get("/flat", async (req, res) => {
  try {
    const [rows, fields] = await pool.query(
      "SELECT flats.*, unit_types.* FROM flats LEFT JOIN unit_types ON flats.unitType = unit_types.id WHERE flats.is_delete = 0 AND flats.floorId = ?;",
      [req.query.floor]
    );
    if (rows.length === 0) {
...
router.patch("/flat/:id", async (req, res) => {
  try {
    ...
    await pool.query(
      "UPDATE flats SET floorId = ?, unitNo = ?, unitType = ?, cleaningFees = ?, description = ?, images = ?, price = ?, bathrooms = ?, beds = ?, guests = ?, deposit = ? WHERE id = ?",
      [
        req.body.floorId,
        req.body.unitNo,
        req.body.unitType,
